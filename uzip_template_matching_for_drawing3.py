...
import matplotlib.dates as mdates  #グラフを作成するライブラリ
...
def Unzip():
    ...
    for audiofile in natsorted(File_List):
        ...
        if rms_A is np.nan:
            ...
            """画像の読み込み"""
            #デフォルト画像の読み込み
            img_DFT = cv2.imread(path_FS + "/ReferenceData/" + "2021121_0:37:46.png")  #".png"の中にリファレンスのファイル名を入力
            #テンプレート画像の読み込み
            ...
            #画像の読み込み
            #デフォルト画像の読み込み
            img_DFT1000 = cv2.imread(path1000Hz + "/ReferenceData/" + "2021121_0:37:46.png")  #".png"の中にリファレンスのファイル名を入力
            #テンプレート画像の読み込み
            ...
            #画像の読み込み
            #デフォルト画像の読み込み
            img_DFT4000 = cv2.imread(path4000Hz + "/ReferenceData/" + "2021121_0:37:46.png")  #".png"の中にリファレンスのファイル名を入力
            #テンプレート画像の読み込み
            ...
            #画像の読み込み
            #デフォルト画像の読み込み
            img_DFT8000 = cv2.imread(path8000Hz + "/ReferenceData/" + "2021121_0:37:46.png")  #".png"の中にリファレンスのファイル名を入力
            #テンプレート画像の読み込み
            ...
            #画像の読み込み
            #デフォルト画像の読み込み
            img_DFT12000 = cv2.imread(path12000Hz + "/ReferenceData/" + "2021121_0:37:46.png")  #".png"の中にリファレンスのファイル名を入力
            #テンプレート画像の読み込み
            ...
            #画像の読み込み
            #デフォルト画像の読み込み
            img_DFT16000 = cv2.imread(path16000Hz + "/ReferenceData/" + "2021121_0:37:46.png")  #".png"の中にリファレンスのファイル名を入力
            #テンプレート画像の読み込み
            ...
            #画像の読み込み
            #デフォルト画像の読み込み
            img_DFT20000 = cv2.imread(path20000Hz + "/ReferenceData/" + "2021121_0:37:46.png")  #".png"の中にリファレンスのファイル名を入力
            #テンプレート画像の読み込み
    ...
    """RMS.読込用の配列を作成"""
    contents1 = []
    csvFile_List1 = glob.glob(path + "*_RMS.csv")
    for csvfile1 in natsorted(csvFile_List1):
        contents1.append(pd.read_csv(csvfile1, header=None))
    
    #1つのデータフレームに結合する    
    #ignore_index=Trueでインデックスをリセット
    df = pd.concat(contents1, ignore_index=True)
    df.to_csv(path+"b.csv", header=False, index=False, mode="a")
    #作成したcsvを読み込む
    df2 = pd.read_csv(path+"b.csv", names=["time","RMS"], encoding="SHIFT_JIS")
    #print(df2)
    #print(pd.to_datetime(df2["time"].astype(str)))
    """グラフ化"""
    x = pd.to_datetime(df2["time"].astype(str))
    #x = pd.to_datetime(df2["time"]).values
    #print(x)
    y = df2["RMS"].values
    plt.scatter(x, y, alpha=0.5)  #透明度0.5
    plt.gca().xaxis.set_major_formatter(mdates.DateFormatter("%Y-%m-%d"))  #横軸を時間表示にする
    plt.gca().xaxis.set_major_locator(mdates.DayLocator(interval=14))  #横軸の目盛り間隔の設定
    plt.xticks(rotation=90, fontsize=8)
    plt.yticks(fontsize=8)
    plt.xlabel("time", fontsize=8)
    plt.ylabel("RMS", fontsize=8)
    plt.grid(axis="y", color="gray", linestyle="--")  #グリッド線はy軸のみ
    plt.subplots_adjust(bottom=0.3)
    plt.savefig(path + timename + "_RMS" + ".png")
    plt.close()
    #print("x",type(x))
    #print("df2_types", df2.dtypes)
    #ファイル削除
    for csvfile1 in natsorted(csvFile_List1):
        os.remove(csvfile1)
    ...
    """類似度.読込用の配列を作成"""
    contents2 = []
    csvFile_List2 = glob.glob(path + "*_FS.csv")
    for csvfile2 in natsorted(csvFile_List2):
        contents2.append(pd.read_csv(csvfile2, header=None))
    
    #1つのデータフレームに結合する    
    #ignore_index=Trueでインデックスをリセット
    df = pd.concat(contents2, ignore_index=True)
    df.to_csv(path+"c.csv", header=False, index=False, mode="a")
    #作成したcsvを読み込む
    df2 = pd.read_csv(path+"c.csv", names=["time","TM"], encoding="SHIFT_JIS")
    #print(df2)
    #print(pd.to_datetime(df2["time"].astype(str)))
    """グラフ化"""
    x = pd.to_datetime(df2["time"].astype(str))
    ...
    plt.yticks(fontsize=8)
    plt.xlabel("time", fontsize=8)
    plt.ylabel("TM", fontsize=8)
    plt.grid(axis="y", color="gray", linestyle="--")  #グリッド線はy軸のみ
    plt.subplots_adjust(bottom=0.3)
    plt.savefig(path + timename + "_TM" + ".png")
    plt.close()
    #print("x",type(x))
    #print("df2_types", df2.dtypes)
    #ファイル削除
...
#関数の読み出しコマンド
#schedule.every().day.at("00:05").do(Unzip)

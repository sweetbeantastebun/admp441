...
def Recording_A():
    ...
    if rms_A is np.nan:
        ...
        t4= time.time()
        #FFT
        N = 8192  #サンプル数を指定 #録音1秒で8192データの周波数分解能は10Hz #4096データなら20Hz
...
def Graph_A1():
    ...
    if rms_A >= threshold_value_MAX:
        fig1.savefig("/home/pi/Documents/admp441_data/"+filename_A+"fig_MAX"".png")
        file =  filename_A + ".wav"
        shutil.copy(path + file , path2)  #wavファイルをコピーして指定ディレクトリへ移動
    """
    if rms_A <= threshold_value_MIN:
        fig1.savefig("/home/pi/Documents/admp441_data/"+filename_A+"fig_MIN"".png")
        file =  filename_A + ".wav"
        shutil.copy(path + file , path2)  #wavファイルをコピーして指定ディレクトリへ移動
    """
    t19 = time.time()

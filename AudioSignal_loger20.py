...
#しきい値を指定
threshold_value_MAX = 0.01
threshold_value_MIN = 0.005
#FFT検出強度のフィルタリング
noise_reduction_filters = 5
...
def Recording_A():
    ...
    if rms_A is np.nan:
       ...
       if rms_A <= 0:
        ...
        t4= time.time()
        #FFT
        N = 8192  #サンプル数を指定 #録音2秒で4096データの周波数分解能は10Hz #8192=5Hz
        ...
        #フィルタリング機能
        spectrum_A[(spectrum_A < noise_reduction_filters)] = 0  #振幅のしきい値未満はゼロにする
        #グラフ準備
...
def job_A():
    ...
    #期間中の波高率
    Wave_height_rate = Average_value / Maximum_value
    """
    #波形の個数を数値化
    data_frame = pd.DataFrame(RMS_data, columns=["spectrum"])  #データフレーム作成
    peeks = (data_frame["spectrum"] >= threshold_value_MAX).sum()  #しきい値以上の検出ピークをカウント
    ...
    #csvファイルに書き込むフレーム作成
    """
    header_names = [["Wave_height_rate", "Average_value", "Maximum_value", "Minimum_value", "number_of_peeks", "memory.percent", "cpu", "disk.percent"],
    [Wave_height_rate, Average_value, Maximum_value, Minimum_value, peeks, memory.percent, cpu, disk.percent]]
    """
    header_names = [["Average_value", "number_of_peeks", "memory.percent", "cpu", "disk.percent"],
    [round(Average_value,4), peeks, memory.percent, cpu, disk.percent]]
    #csv作成
    with open("/home/pi/Documents/admp441_data/"+filename_A+"production_routine"".csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerows(header_names)
    """
    #wavファイル削除

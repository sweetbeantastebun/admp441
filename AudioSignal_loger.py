...
def RMS_A():
    ...
    #FFT
    N = 8192  #サンプル数を指定 #録音2秒で4096データの周波数分解能は10Hz #8192=20Hz
...
def Graph_A():
    ...
    plt.axis([0,index_loop, 0,0.05])
    plt.xticks(fontsize = 8)
    plt.yticks(fontsize = 8)
    plt.xlabel("Sample Number", fontsize=9)
    plt.ylabel("RMS", fontsize=9)
    plt.grid(which="both")
    No1.set_data(np.arange(0, index_loop), RMS_data)

    #plt.subplot(2, 1, 2)
    #No2, = plt.plot(np.arange(frequency_A, np.abs(spectrum_A), lw=2)
    #plt.axis([0,1400, 0,40])
    #plt.xticks(fontsize = 8)
    #plt.yticks(fontsize = 8)
    #plt.grid(which="both")
    #plt.xlabel("freqency(Hz)", fontsize=9)
    #plt.ylabel("Amplitude Spectrum", fontsize=9)
    #No2.set_data(frequency_A, np.abs(spectrum_A))
